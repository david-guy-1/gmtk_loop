...
function drawRoundedRectangle(
  ...
  var ccw =
    Math.cos(angle + Math.PI / 2) * (x0 - x1) +
      Math.sin(angle + Math.PI / 2) * (y0 - y1) >
    0
...
function cell_index(top_left, w, h, amt_per_row, x, y) {
  ...
  let [p, q] = [
    Math.floor((x - top_left[0]) / w),
    Math.floor((y - top_left[1]) / h)
  ]
  if (p >= amt_per_row) {
    return undefined
  }
...
function between(x, b1, b2) {
  // returns if x is between b1 and b2  (inclusive:number)
  noNaN(arguments)
  if (b1 <= x && x <= b2) {
    return true
  }
...
function doLinesIntersect(...args) {
  ...
  return (
    between(intersectionPoint[0], p1x, p2x) &&
...
function getLineEndWH(...args) {
  ...
  let a, b, c
  if (p2y - p1y != 0) {
    // a is not 0, set a = 1 (use this chart)
  ...
  for (let i = 0; i < 4; i++) {
    ...
    try {
      ...
      if (
        ...
        if (
          (intersection[0] - p1x) * (p2x - p1x) +
            (intersection[1] - p1y) * (p2y - p1y) >=
          0
        ) {
          return intersection
        }
...
function corners(tlx, tly, width, height, angle) {
  ...
  cornersLst.push([
    cornersLst[0][0] + width * Math.cos(angle),
    cornersLst[0][1] + width * Math.sin(angle)
  ])

  //travel "upwards" (height) units along angle- 90 degrees
  cornersLst.push([
    cornersLst[1][0] + height * Math.cos(angle + Math.PI / 2),
    cornersLst[1][1] + height * Math.sin(angle + Math.PI / 2)
  ])

  //travel "upwards" from the start
  cornersLst.push([
    cornersLst[0][0] + height * Math.cos(angle + Math.PI / 2),
    cornersLst[0][1] + height * Math.sin(angle + Math.PI / 2)
  ])
